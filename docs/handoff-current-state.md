...
- `src/lib/server/pipeline-runner.ts` — ジョブのパイプライン実行ライフサイクル管理（queued → processing → completed/failed）
- `src/lib/server/ifc-generator.ts` — PipelineOutput から最小 IFC を生成し artifact 保存
...
| IFC 高品質化 | 最小実装済み（`data/artifacts/<jobId>/model.ifc`）。opening/属性の厳密化は未対応 |
...
src/lib/server/pipeline-runner.ts  # ジョブのパイプライン実行ライフサイクル管理
